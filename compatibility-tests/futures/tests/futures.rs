...
mod api {
    ...
    pub fn keep_fetching_page<'u>(url: &'u str) -> impl Stream<Item = Result<String, Error>> + 'u {
...
use snafu::{
    futures::{future::FutureExt as _, stream::StreamExt as _},
    ResultExt, Snafu,
};
...
// Can also be used as a `Future` combinator
...
// Can also be used as a `Stream` combinator
